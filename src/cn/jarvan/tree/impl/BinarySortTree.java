 ...
 * （3）左、右子树也分别为二分查找树；
 * 注意：新增和查询二叉树都不难，而删除二叉树却要分为三种情形
 * 1、删除的节点为叶子节点：直接置为null
 * 2、删除的节点仅有左孩子或者右孩子：将改节点置为不为空的孩子
 * 3、删除的节点既有左还有又有右孩子：找前继节点，删除前继节点，用前继节点替换被删除的节点
 * 注意点：
 * 1、一定先删除前继节点再替换，若先替换再删除可能把已替换的节点当做前继节点给删除了
 * 2、替换节点时需要对该节点的父节点的左右孩子引用进行操作，如果替换节点是根节点，则无父节点，需考虑这种情况
 *
 * <p>
